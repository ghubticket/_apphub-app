...
export const createEvent = async (req: Request, res: Response) => {
    try {
        const { name, description, date, time, location, address, city, state } = req.body
        ...
        res.status(400).json({ success: false, message: 'Erro ao criar evento', errors: [error.message] })
    }
}
