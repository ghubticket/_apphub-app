...
export const startReservationExpirationScheduler = (): void => {
    console.log(`🕒 Reservation expiration scheduler ativo (interval=${CLEANUP_INTERVAL_MS}ms)`);
    ...
    setInterval(async () => {
        try {
            ...
            if (result.success && result.cleanedCount > 0) {
                console.log(
                    `✅ ${result.cleanedCount} reserva(s) expirada(s) cancelada(s) automaticamente`
                );
            }
        } catch (error) {
