...
async function scanAndValidateQR(
    qr: string,
    jwt: string,
    baseUrl: string
): Promise<{ success: boolean; ticketCode?: string; alreadyUsed?: boolean }> {
    try {
...
async function main() {
    ...
    const orders = await Order.find(query).populate('tickets').lean();
