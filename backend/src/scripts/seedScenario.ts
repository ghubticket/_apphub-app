...
async function main() {
  ...
  const user = await (async () => {
    ...
    if (!any) throw new Error('Nenhum usuário encontrado. Crie um usuário antes de rodar este script.');
    return any;
  })();
  ...
  try {
    ...
    if (String(e?.message || '').includes('Código já existe') || String(e?.code) === '11000') {
      promo = await PromoterCode.findOne({ code: 'PROMO10', deletedAt: null }) as unknown as IPromoterCode | null;
      if (!promo) throw e;
      // garantir associação ao evento
      const hasEvent = (promo!.events || []).some((ev: any) => String(ev) === String(event._id));
      if (!hasEvent) {
  ...
  async function createOrderOneTicket(tt: any, promoterCode?: string, intendedMethod?: 'pix' | 'credit_card' | 'debit_card' | 'bank_slip' | 'vip_free') {
    const subtotal = tt.price;
    const discount = promoterCode ? (promo!.discountType === 'percentage' ? Math.round((subtotal * promo!.discountValue) / 100) : promo!.discountValue) : 0;
    const platformFee = Math.round(((subtotal - discount) * (event.platformFeePercentage || 0)) / 100);
  ...
  try {
    ...
    const sandboxEmail = !isProd && originalEmail && !originalEmail.endsWith('@testuser.com')
    ...
    const pix = await paymentService.createPixPayment({
      ...
      items: [{ title: pista.name, description: pista.description, quantity: 1, unit_price: pista.price, category: 'tickets' }],
    }, deviceId);
    ...
    console.warn('⚠️ Não foi possível iniciar PIX automaticamente. Você pode usar a rota /api/payments/{orderId}/pix');
  }
  ...
  console.log('👉 Para pagar via cartão use a rota: POST /api/payments/' + cardOrder._id + '/card com { token, paymentMethodId, installments }');
