...
function printUsage() {
    console.log(`
Uso: npm run ts-node src/scripts/createAdminUser.ts -- --email admin@dominio.com --password SenhaForte123! [--name "Nome do Admin"]

Argumentos:
  --email      (obrigatório) Email do usuário admin
  --password   (obrigatório) Senha que será definida para o admin
  --name       (opcional)    Nome completo. Padrão: "Administrador 5521"
`);
}
...
async function main() {
    ...
    const name = args.name || args.n || 'Administrador 5521';
    ...
    try {
        await upsertAdminUser({ name, email, password });
        console.log('🎉 Operação concluída com sucesso.');
    } catch (error: any) {
        console.error('❌ Erro ao criar/atualizar usuário admin:', error?.message || error);
        process.exitCode = 1;
    } finally {
