...
export const validateMobileDevice = (req: Request, res: Response, next: NextFunction) => {
    ...
    if (process.env.NODE_ENV === 'development') {
        console.warn('⚠️ Acesso de dispositivo não-mobile detectado (permitido apenas em dev):', userAgent);
        return next();
    }
    ...
    return res.status(403).json({
        ...
        errors: ['Este endpoint é exclusivo para dispositivos móveis e tablets. Por favor, acesse de um celular ou tablet.'],
    });
};
