        ...
        <div className={`fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 transition-all duration-300 ${entering ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
            <div className={`bg-white rounded-2xl shadow-2xl max-w-md w-full p-6 transition-all duration-300 ${activeClass}`}>
                ...
                <div className="flex md:flex-row flex-col md:justify-end gap-3">
                    ...
                    <button
                        onClick={() => {
                            // Esconde o aviso imediatamente para evitar "loop" de modais
                            // e deixa o hook cuidar da renovação em background.
                            dismissWarning();
                            refreshSession();
                        }}
                        disabled={isRefreshing}
                        className="px-4 py-2 text-sm font-medium text-white bg-blue-600 justify-center rounded-md hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 disabled:opacity-50 disabled:cursor-not-allowed transition-colors flex items-center gap-2"
                    >
                        {isRefreshing ? (
                            <>
                                <svg
