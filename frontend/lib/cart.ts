...
const sanitizeCartItem = (item: CartItem): CartItem => {
  const metaAllowInstallments =
    item.metadata &&
    (item.metadata.allowInstallments === true ||
      item.metadata.allowInstallments === "true");
  const metaMinInstallments =
    item.metadata && typeof item.metadata.minInstallments === "number"
      ? (item.metadata.minInstallments as number)
      : undefined;
  const metaMaxInstallments =
    item.metadata && typeof item.metadata.maxInstallments === "number"
      ? (item.metadata.maxInstallments as number)
      : undefined;

  const sanitized = {
    id: String(item.id),
    ticketTypeId: item.ticketTypeId
      ? String(item.ticketTypeId)
      : String(item.id),
    ...
    maxQuantity:
      ...
      item.maxQuantity > 0
        ...
        : undefined,
    ticketFee:
      typeof item.ticketFee === "number" && Number.isFinite(item.ticketFee)
        ? Number(item.ticketFee)
        : undefined,
    platformFeePercentage:
      typeof item.platformFeePercentage === "number" &&
      Number.isFinite(item.platformFeePercentage)
        ? Number(item.platformFeePercentage)
        : undefined,
    // Se allowInstallments vier undefined, tentar usar metadata
    allowInstallments:
      item.allowInstallments === true || metaAllowInstallments === true,
    minInstallments:
      typeof item.minInstallments === "number" &&
      Number.isFinite(item.minInstallments)
        ? item.minInstallments
        : typeof metaMinInstallments === "number"
        ? metaMinInstallments
        : item.minInstallments ?? null,
    maxInstallments:
      typeof item.maxInstallments === "number" &&
      Number.isFinite(item.maxInstallments)
        ? item.maxInstallments
        : typeof metaMaxInstallments === "number"
        ? metaMaxInstallments
...
export const saveCartItems = (items: CartItem[]) => {
  ...
  window.localStorage.setItem(CART_STORAGE_KEY, jsonString);
...
export const addCartItem = (
  ...
  const existingIndex = current.findIndex(
    (cartItem) => cartItem.id === sanitized.id
  );
  if (existingIndex >= 0) {
