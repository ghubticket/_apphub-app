...
export function useCheckoutCustomer() {
  ...
  useEffect(() => {
    ...
    if (savedUserId && savedUserId !== userId) {
      ...
      setCustomerData({
        ...
        billingStreet: savedData.billingStreet || "",
        billingNumber: savedData.billingNumber || "",
        billingNeighborhood: savedData.billingNeighborhood || "",
        billingCity: savedData.billingCity || "",
        billingState: savedData.billingState || "",
        billingZip: savedData.billingZip || "",
      });
    } else {
  ...
  useEffect(() => {
    ...
    const hasAnySavedData =
      savedData &&
      (savedData.name || savedData.email || savedData.cpf || savedData.phone);
    ...
    setCustomerData((prev) => {
      ...
      const allFieldsEmpty =
        !next.name && !next.email && !next.cpf && !next.phone;
  ...
  const handleChange = useCallback(
    (field: keyof CheckoutCustomerData, value: string) => {
      ...
      setCustomerData((prev) => {
        ...
        switch (field) {
          ...
          case "phone":
            ...
            break;
          case "billingZip":
            // Manter apenas dígitos e aplicar máscara simples de CEP
            {
              ...
              const formatted =
                digits.length > 5
                  ...
                  : digits;
              next.billingZip = formatted;
            }
            break;
          case "billingState":
            next.billingState = value.toUpperCase().slice(0, 2);
            break;
          case "billingStreet":
          case "billingNumber":
          case "billingNeighborhood":
          case "billingCity":
            next[field] = value;
            break;
        }
