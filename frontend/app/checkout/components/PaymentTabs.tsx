...
export function PaymentTabs({
    ...
    return (
        <div className="flex flex-col md:flex-row gap-3">
            {/* Primeira linha: CARTÃO e PIX (mobile e desktop) */}
            ...
            {/* Segunda linha: PARCELAMENTO DISPONÍVEL (100% no mobile, flex-1 no desktop) */}
            {showInstallmentsTab && (
                <button
                    type="button"
                    onClick={() => {
                    ...
                    disabled={pixPaymentActive}
                    aria-disabled={pixPaymentActive}
                    className={`w-full md:flex-1 flex-column rounded-full border px-5 py-3 text-xs font-light uppercase tracking-normal transition disabled:cursor-not-allowed disabled:opacity-60 disabled:shadow-none ${selectedTab === 'parcel'
                            ? 'border-[#1a1a1d] bg-[#1a1a1d] text-white'
                            : 'border-[#ded7ca] bg-[#faf7f0] text-[#4c4c55] hover:border-[#a38f78]'
                        }`}
                >
